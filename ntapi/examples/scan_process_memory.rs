...
impl MemoryBasicInformation {
    ...
    fn get_mapped_file_name(handle: HANDLE, base_address: PVOID) -> [u16; MAX_MODULE_NAME_LEN] {
        unsafe {
            ...
            if status < 0 {
                return [0u16; MAX_MODULE_NAME_LEN];
            }
    ...
    pub fn mapped_name(&self) -> &[u16] {
        let len = (0..MAX_MODULE_NAME_LEN)
            .find(|&i| self.mapped_name[i] == 0)
            .unwrap_or(0);
        &self.mapped_name[..len]
    }
...
impl fmt::Display for MemoryBasicInformation {
        ...
        if self.is_image() {
            ...
            if !name.is_empty() {
                ...
                for &ch in name {
                    if ch == 0 { break; }
                    if ch >= 32 && ch <= 126 {
                        write!(f, "{}", char::from_u32(ch as u32).unwrap_or('?'))?;
                    }
                }
            }
        }
...
impl Iterator for MemoryRegionIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        unsafe {
            ...
            Some(MemoryBasicInformation::new(self.handle, mbi))
        }
    }
}
